...
export default {
  ...
  methods:{
    stoken(){//输入token点击登录
      let $eit=this;
       this.$http.post(apis+'/accesstoken',{
    ...
    show_child(index){//点击展开列表
      if(this.loginSuccess[index].numberLength>0){
        this.hui_zong_i=index;
        this.loginSuccess[index].huifu_zong=!this.loginSuccess[index].huifu_zong;
      }
      console.log(index)
    },
    obtain(){//每次刷新获取本地token登录
    // alert(apis+'/user/'+localStorage.getItem('loginname'))
        let $eit = this;
        this.$http.get(apis+'/user/'+localStorage.getItem('loginname'))
      .then((response) =>{
        if(response.status == 200){
           console.log('刷新页面返回的========')
            console.log(response.data.data);
          $eit.userAngth=response.data.data;
          $eit.loginSuccess[0].numberLength=response.data.data.recent_topics.length;
          $eit.loginSuccess[1].numberLength=response.data.data.recent_replies.length;
          $eit.loginSuccess[2].numberLength=response.data.data.collect_topics.length;
